var Cjs = {
  stringToColor: function stringToColor(str, opts){
	...
	dateStringReplacements: {
		...
		'%c': function(date){ return Cjs.dateStringReplacements['%x'](date) +' '+ Cjs.dateStringReplacements['%X'](date); },
		'%d': function(date){ return Cjs.padNumber(date.getDate(), 2); },
		'%H': function(date){ return Cjs.padNumber(date.getHours(), 2); },
		'%I': function(date){ return Cjs.padNumber((date.getHours() + 12) % 12, 2); },
		'%j': function(date){
			var dayCount = [0, 31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334];
			var month = date.getMonth(), day = date.getDate();
			var dayOfYear = dayCount[month] + day;

			if(month > 1 && Cjs.isLeapYear(date)) dayOfYear++;

			return dayOfYear;
		},
		'%m': function(date){ return Cjs.padNumber(date.getMonth() + 1, 2); },
		'%M': function(date){ return Cjs.padNumber(date.getMinutes(), 2); },
		'%p': function(date){ return date.getHours() < 12 ? 'AM' : 'PM'; },
		'%S': function(date){ return Cjs.padNumber(date.getSeconds(), 2); },
		'%U': function(date){
		...
		'%x': function(date){ return Cjs.dateStringReplacements['%m'](date) +'/'+ Cjs.dateStringReplacements['%d'](date) +'/'+ Cjs.dateStringReplacements['%y'](date); },
		'%X': function(date){ return Cjs.dateStringReplacements['%H'](date) +':'+ Cjs.dateStringReplacements['%M'](date) +':'+ Cjs.dateStringReplacements['%S'](date); },
	...
	parseDateString: function parseDateString(format){
		...
		var matches = Cjs.uniqueArr(format.match(/%./g));
		var x = 0, count = matches.length, replacement;
		var date = new Date();

		for(; x < count; ++x){
			replacement = Cjs.dateStringReplacements[matches[x]];
	...
	randColor: function(opts){
		...
		var h = Cjs.randInt(Math.max(0, h_lvl - h_var), Math.min(360, h_lvl + h_var));

		var s_lvl = opts.s_lvl || 55;
		var s_var = opts.s_var || 25;
		var s = Cjs.randInt(Math.max(0, s_lvl - s_var), Math.min(100, s_lvl + s_var));

		var l_lvl = opts.l_lvl || 55;
		var l_var = opts.l_var || 10;
		var l = Cjs.randInt(Math.max(0, l_lvl - l_var), Math.min(100, l_lvl + l_var));
	...
	runInOrder: function(functions, currentFunction){
		...
		var next = function(){
			Cjs.runInOrder(functions, nextFunction);
		};
